defmodule Homelab.Deployments.Readiness do
  ...
  # What the donor's env ACTUALLY says right now — an operator override included. The
  # derived value only reaches the container on a re-deploy, so reading the intent
  # rather than the state would report a route as working before it is.
