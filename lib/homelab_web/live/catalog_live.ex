defmodule HomelabWeb.CatalogLive do
  ...
  # The Workbench links here as `/catalog?template=<id>` to configure & deploy a
  # freshly-built image: open the deploy modal for that template.
  @impl true
  def handle_params(%{"template" => template_id}, _uri, socket) do
    case Integer.parse(template_id) do
      {id, _} ->
        case Catalog.get_app_template(id) do
          {:ok, template} ->
            {:noreply,
             socket
      ...
      <div>
        <div class="mb-5">
          ...
          <p class="mt-1 text-sm text-base-content/50">
            Search any registry, deploy a custom image, or browse curated sources.
          </p>
        </div>
