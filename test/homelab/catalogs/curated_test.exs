defmodule Homelab.Catalogs.CuratedTest do
  ...
  describe "kernel privileges" do
    ...
    test "a catalog volume can nominate a host folder rather than a managed volume" do
      # Without type/source every catalog app defaulted to a managed volume — correct for
      # an app's own state, wrong for the class of app whose point is reading data the
      # operator already has. The catalog could not express the difference at all.
      assert {:ok, entry} = Curated.app_details("Gitea")

      assert Enum.all?(entry.required_volumes, &Map.has_key?(&1, "type"))
      assert Enum.all?(entry.required_volumes, &(&1["type"] == "volume"))
    end
