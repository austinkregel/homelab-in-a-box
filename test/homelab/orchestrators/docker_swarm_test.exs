defmodule Homelab.Orchestrators.DockerSwarmTest do
  ...
  describe "deploy/1 (mocked daemon)" do
    ...
    # The Engine driver has asserted this by name for a long time; Swarm had no
    # equivalent test and quietly did the opposite, joining ingress on `service_mode` as
    # well as `traefik.enable`. "Internal only" placed the container on the network
    # Traefik and every other tenant's routed workload share.
    test "an Internal-only datastore is NOT attached to the shared ingress network" do
      test_pid = self()
      stub(Homelab.Mocks.DockerClient, :post_stream, fn _path, _opts -> :ok end)
      ...
      spec =
        build_spec()
        |> Map.put(:service_mode, true)
        |> Map.put(:labels, %{"homelab.managed" => "true"})

      assert {:ok, "svc1"} = DockerSwarm.deploy(spec)

      assert_received {:create_body, body}
      targets = get_in(body, ["TaskTemplate", "Networks"]) |> Enum.map(& &1["Target"])

      assert spec.network in targets
      refute "homelab-iab-internal" in targets
    end

    test "a ROUTED workload still joins ingress" do
      test_pid = self()
      stub(Homelab.Mocks.DockerClient, :post_stream, fn _path, _opts -> :ok end)
